...
const DropdownField = ({
  ...
  const onSelect = (event: SelectChangeEvent) => {
    const selectedItem = items.find((item: any) => item._id === event.target.value);
    ...
    if(items != undefined) {
      items.map((item) => {
        ...
        if(Array.isArray(customLabel)){
          ...
          temp.push({ 
            label: item[customLabel] ? item[customLabel] : item._id, 
            value: item._id
          })
        }
      })
  ...
  return (
    <Box>
      <FormControl margin="normal" fullWidth variant="outlined" disabled={disabled} error={hasError}>
        ...
        <Select
          value={value?._id && dropdownItems.length > 0 ? value?._id : ''}
