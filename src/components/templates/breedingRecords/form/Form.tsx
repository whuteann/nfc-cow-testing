...
interface FormProps {
  record?: Prisma.BirthRecordCreateInput;
...
function BreedingForm({ 
  ...
  useEffect(() => {
    if (session != undefined && initLoad != true) {
      setInitLoad(true);
      record? getCows(record.farm as any) : setCows([])
  ...
  const createRecord = (values: any, setSubmitting: any) => {
    return create(values, () => {
      ...
      router.push(`/dashboard/farm-birth-records`);
    }, (error: any) => {
  ...
  const getCows = (f: Prisma.FarmCreateInput, setFieldValue: any = null) => {
    let query = {
      gender: 'Female',
      farm: f.name, 
      filterCountry: true,
          ...
          <Grid container spacing={3}>
            <Grid item xs={12}>
              <Form>
                ...
                {
                  (values.option === 'Select By NFC ID' && values.farm !== "") || record
                  ? 
                    <DropdownField
                      label={t('Cow NFC ID')}
                      customLabel='nfcId'
                ...
                {
                  (values.option === 'Select By Photo' && values.farm !== "") || record
                  ? 
                    <DropdownImage
                      label={t('Cow Image')}
                      disabled={record ? true : false}
